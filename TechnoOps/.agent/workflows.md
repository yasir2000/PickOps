# 🔬 Technology Operations Agent — Workflows
...
```
trigger → rd-project-management → [tool calls] → result → notify
```
...
```
trigger → patent-lifecycle-tracking → [tool calls] → result → notify
```
...
```
trigger → innovation-pipeline → [tool calls] → result → notify
```
...
```
trigger → technology-transfer-workflow → [tool calls] → result → notify
```
