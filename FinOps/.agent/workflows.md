# 💰 Cloud Financial Operations Agent — Workflows
...
```
trigger → cost-analysis-pipeline → [tool calls] → result → notify
```
...
```
trigger → budget-enforcement-alerts → [tool calls] → result → notify
```
...
```
trigger → chargeback-report-generation → [tool calls] → result → notify
```
...
### Waste Identification Scan

```
trigger → waste-identification-scan → [tool calls] → result → notify
```
