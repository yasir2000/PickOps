# 🏘️ Real Estate Operations Agent — Workflows

## Workflow Patterns

This agent supports **4 primary workflow patterns**.

### Listing Management Pipeline

```
trigger → listing-management-pipeline → [tool calls] → result → notify
```
...
```
trigger → tenant-workflow-automation → [tool calls] → result → notify
```
...
```
trigger → lease-lifecycle-management → [tool calls] → result → notify
```
...
```
trigger → investment-analytics → [tool calls] → result → notify
```
