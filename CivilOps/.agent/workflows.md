# 🏗️ Civil Engineering Operations Agent — Workflows
...
```
trigger → structural-monitoring-alert → [tool calls] → result → notify
```
...
```
trigger → project-milestone-tracker → [tool calls] → result → notify
```
...
```
trigger → asset-condition-report → [tool calls] → result → notify
```
...
```
trigger → compliance-audit-pipeline → [tool calls] → result → notify
```
