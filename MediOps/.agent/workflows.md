# 📰 Media Operations Agent — Workflows
...
```
trigger → asset-management-workflow → [tool calls] → result → notify
```
...
```
trigger → publishing-automation → [tool calls] → result → notify
```
...
```
trigger → audience-analytics → [tool calls] → result → notify
```
