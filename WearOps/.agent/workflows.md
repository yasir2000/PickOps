# ⌚ Wearable Technology Operations Agent — Workflows
...
```
trigger → health-data-pipeline → [tool calls] → result → notify
```
...
```
trigger → ota-update-rollout → [tool calls] → result → notify
```
...
```
trigger → privacy-compliance-check → [tool calls] → result → notify
```
...
```
trigger → health-anomaly-detection → [tool calls] → result → notify
```
