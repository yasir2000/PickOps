# 📚 Publishing Operations Agent — Workflows

## Workflow Patterns

This agent supports **4 primary workflow patterns**.

### Manuscript Submission Pipeline

```
trigger → manuscript-submission-pipeline → [tool calls] → result → notify
```
...
```
trigger → editorial-workflow → [tool calls] → result → notify
```
...
```
trigger → production-pipeline → [tool calls] → result → notify
```
...
```
trigger → distribution-management → [tool calls] → result → notify
```
