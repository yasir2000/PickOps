# 🛍️ Retail Operations Agent — Workflows
...
```
trigger → inventory-optimization-pipeline → [tool calls] → result → notify
```
...
```
trigger → dynamic-pricing-engine → [tool calls] → result → notify
```
...
```
trigger → customer-personalization → [tool calls] → result → notify
```
...
```
trigger → supply-chain-coordination → [tool calls] → result → notify
```
