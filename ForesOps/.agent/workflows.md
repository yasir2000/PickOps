# 🌲 Forestry Operations Agent — Workflows
...
```
trigger → inventory-management-pipeline → [tool calls] → result → notify
```
...
```
trigger → fire-risk-monitoring → [tool calls] → result → notify
```
...
```
trigger → reforestation-tracker → [tool calls] → result → notify
```
...
```
trigger → carbon-credit-reporting → [tool calls] → result → notify
```
