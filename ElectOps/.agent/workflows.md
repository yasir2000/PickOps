# ⚡ Electronics Operations Agent — Workflows

## Workflow Patterns

This agent supports **4 primary workflow patterns**.

### Pcb Design Versioning

```
trigger → pcb-design-versioning → [tool calls] → result → notify
```
...
```
trigger → test-automation-pipeline → [tool calls] → result → notify
```
...
```
trigger → component-tracking → [tool calls] → result → notify
```
...
```
trigger → failure-analysis-report → [tool calls] → result → notify
```
