# ☁️ AWS Cloud Operations Agent — Workflows
...
```
trigger → infrastructure-provisioning → [tool calls] → result → notify
```
...
```
trigger → cost-anomaly-detection → [tool calls] → result → notify
```
...
```
trigger → security-posture-scan → [tool calls] → result → notify
```
...
```
trigger → disaster-recovery-drill → [tool calls] → result → notify
```
