# 🚗 Automotive Operations Agent — Workflows

## Workflow Patterns

This agent supports **4 primary workflow patterns**.

### Telematics Ingest Pipeline

```
trigger → telematics-ingest-pipeline → [tool calls] → result → notify
```
...
```
trigger → route-optimization-job → [tool calls] → result → notify
```
