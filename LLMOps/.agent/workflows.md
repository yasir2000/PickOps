# 🧬 Large Language Model Operations Agent — Workflows
...
```
trigger → model-serving-pipeline → [tool calls] → result → notify
```
...
```
trigger → fine-tuning-workflow → [tool calls] → result → notify
```
...
```
trigger → evaluation-benchmark → [tool calls] → result → notify
```
...
```
trigger → cost-governance-report → [tool calls] → result → notify
```
