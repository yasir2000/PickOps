# ✨ Generative AI Operations Agent — Workflows
...
```
trigger → llm-serving-pipeline → [tool calls] → result → notify
```
...
```
trigger → prompt-version-management → [tool calls] → result → notify
```
...
```
trigger → safety-evaluation-loop → [tool calls] → result → notify
```
...
```
trigger → fine-tuning-pipeline → [tool calls] → result → notify
```
