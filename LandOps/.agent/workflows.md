# 🗺️ Land Management Operations Agent — Workflows
...
```
trigger → land-record-management → [tool calls] → result → notify
```
...
### Gis Mapping Pipeline

```
trigger → gis-mapping-pipeline → [tool calls] → result → notify
```
...
```
trigger → environmental-monitoring → [tool calls] → result → notify
```
