# 📚 Retrieval-Augmented Generation Operations Agent — Workflows
...
```
trigger → document-ingestion-pipeline → [tool calls] → result → notify
```
...
```
trigger → vector-store-management → [tool calls] → result → notify
```
...
```
trigger → retrieval-evaluation → [tool calls] → result → notify
```
...
```
trigger → embedding-model-update → [tool calls] → result → notify
```
