# 🎯 Service Operations Agent — Workflows
...
```
trigger → sla-monitoring-pipeline → [tool calls] → result → notify
```
...
```
trigger → ticket-routing-automation → [tool calls] → result → notify
```
...
```
trigger → customer-success-workflow → [tool calls] → result → notify
```
...
```
trigger → csat-measurement → [tool calls] → result → notify
```
