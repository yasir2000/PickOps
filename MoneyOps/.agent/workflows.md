# 💵 Money Operations Agent — Workflows

## Workflow Patterns

This agent supports **4 primary workflow patterns**.

### Budget Automation Pipeline

```
trigger → budget-automation-pipeline → [tool calls] → result → notify
```
...
```
trigger → expense-categorization → [tool calls] → result → notify
```
...
```
trigger → cash-flow-forecast → [tool calls] → result → notify
```
...
```
trigger → tax-preparation-workflow → [tool calls] → result → notify
```
