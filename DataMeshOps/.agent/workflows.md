# 🕸️ Data Mesh Operations Agent — Workflows
...
```
trigger → data-product-registration → [tool calls] → result → notify
```
...
```
trigger → contract-validation-pipeline → [tool calls] → result → notify
```
...
```
trigger → quality-monitoring-loop → [tool calls] → result → notify
```
...
```
trigger → lineage-tracking → [tool calls] → result → notify
```
