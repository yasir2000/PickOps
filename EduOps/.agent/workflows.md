# 🎓 Education Operations Agent — Workflows
...
```
trigger → student-analytics-pipeline → [tool calls] → result → notify
```
...
```
trigger → adaptive-learning-engine → [tool calls] → result → notify
```
...
```
trigger → curriculum-planning-workflow → [tool calls] → result → notify
```
...
```
trigger → accreditation-audit → [tool calls] → result → notify
```
