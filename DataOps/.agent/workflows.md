# 📊 Data Operations Agent — Workflows
...
```
trigger → pipeline-orchestration → [tool calls] → result → notify
```
...
```
trigger → quality-gate-validation → [tool calls] → result → notify
```
...
```
trigger → catalog-sync-pipeline → [tool calls] → result → notify
```
...
```
trigger → lineage-capture → [tool calls] → result → notify
```
