# 🏦 Banking Operations Agent — Workflows
...
```
trigger → payment-processing-pipeline → [tool calls] → result → notify
```
...
```
trigger → fraud-detection-model → [tool calls] → result → notify
```
...
```
trigger → aml-screening-loop → [tool calls] → result → notify
```
