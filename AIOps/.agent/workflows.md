# 🧠 AI for IT Operations Agent — Workflows
...
```
trigger → anomaly-detection-pipeline → [tool calls] → result → notify
```
...
```
trigger → alert-correlation-engine → [tool calls] → result → notify
```
...
```
trigger → auto-remediation-loop → [tool calls] → result → notify
```
...
```
trigger → capacity-forecast-report → [tool calls] → result → notify
```
