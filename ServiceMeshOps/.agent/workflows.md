# 🕸️ Service Mesh Operations Agent — Workflows
...
```
trigger → traffic-management-pipeline → [tool calls] → result → notify
```
...
```
trigger → mtls-enforcement → [tool calls] → result → notify
```
...
```
trigger → mesh-observability → [tool calls] → result → notify
```
...
### Canary Routing Setup

```
trigger → canary-routing-setup → [tool calls] → result → notify
```
