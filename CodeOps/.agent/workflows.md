# 💻 Code Operations Agent — Workflows
...
```
trigger → pr-quality-gate → [tool calls] → result → notify
```
...
```
trigger → dependency-scan-pipeline → [tool calls] → result → notify
```
...
```
trigger → dora-metrics-dashboard → [tool calls] → result → notify
```
...
```
trigger → debt-remediation-sprint → [tool calls] → result → notify
```
