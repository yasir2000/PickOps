# 🧠 Psychology & Mental Health Operations Agent — Workflows
...
```
trigger → clinical-workflow-automation → [tool calls] → result → notify
```
...
```
trigger → patient-monitoring-pipeline → [tool calls] → result → notify
```
...
```
trigger → crisis-intervention-routing → [tool calls] → result → notify
```
...
```
trigger → telehealth-operations → [tool calls] → result → notify
```
