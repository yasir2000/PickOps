# 🚀 Space Operations Agent — Workflows
...
```
trigger → telemetry-processing-pipeline → [tool calls] → result → notify
```
...
```
trigger → satellite-health-monitoring → [tool calls] → result → notify
```
...
```
trigger → mission-planning-workflow → [tool calls] → result → notify
```
...
```
trigger → ground-station-ops → [tool calls] → result → notify
```
