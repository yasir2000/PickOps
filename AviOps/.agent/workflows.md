# 🛩️ Avionics Operations Agent — Workflows
...
```
trigger → certification-pipeline → [tool calls] → result → notify
```
...
```
trigger → firmware-ota-rollout → [tool calls] → result → notify
```
...
### Fault Isolation Loop

```
trigger → fault-isolation-loop → [tool calls] → result → notify
```
...
```
trigger → compliance-traceability-report → [tool calls] → result → notify
```
