# 🤖 Machine Learning Operations Agent — Workflows
...
```
trigger → training-pipeline → [tool calls] → result → notify
```
...
```
trigger → model-registration-workflow → [tool calls] → result → notify
```
...
```
trigger → serving-deployment → [tool calls] → result → notify
```
...
```
trigger → drift-detection-monitor → [tool calls] → result → notify
```
