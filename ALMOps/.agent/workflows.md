# 🔄 Application Lifecycle Management Operations Agent — Workflows
...
```
trigger → change-request-approval → [tool calls] → result → notify
```
...
```
trigger → release-gate-pipeline → [tool calls] → result → notify
```
...
```
trigger → compliance-audit-report → [tool calls] → result → notify
```
...
### Debt Measurement Sprint

```
trigger → debt-measurement-sprint → [tool calls] → result → notify
```
