# 🏭 Manufacturing Operations Agent — Workflows
...
```
trigger → production-planning-pipeline → [tool calls] → result → notify
```
...
```
trigger → qc-automation-gate → [tool calls] → result → notify
```
...
```
trigger → mes-integration-workflow → [tool calls] → result → notify
```
...
### Oee Monitoring Report

```
trigger → oee-monitoring-report → [tool calls] → result → notify
```
