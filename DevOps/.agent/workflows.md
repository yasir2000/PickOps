# 🔧 Development Operations Agent — Workflows
...
```
trigger → ci-cd-pipeline → [tool calls] → result → notify
```
...
```
trigger → iac-provisioning → [tool calls] → result → notify
```
...
```
trigger → release-gate-approval → [tool calls] → result → notify
```
...
```
trigger → k8s-deployment-rollout → [tool calls] → result → notify
```
