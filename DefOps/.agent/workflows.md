# 🛡️ Defence Operations Agent — Workflows

## Workflow Patterns

This agent supports **4 primary workflow patterns**.

### Secure Comms Pipeline

```
trigger → secure-comms-pipeline → [tool calls] → result → notify
```
...
```
trigger → mission-planning-automation → [tool calls] → result → notify
```
...
```
trigger → cyber-threat-monitoring → [tool calls] → result → notify
```
...
```
trigger → compliance-audit → [tool calls] → result → notify
```
