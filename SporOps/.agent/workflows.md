# ⚽ Sports Operations Agent — Workflows
...
```
trigger → performance-analytics-pipeline → [tool calls] → result → notify
```
...
```
trigger → team-management-workflow → [tool calls] → result → notify
```
...
```
trigger → fan-engagement-automation → [tool calls] → result → notify
```
...
```
trigger → venue-operations → [tool calls] → result → notify
```
