# 🐔 Poultry Operations Agent — Workflows
...
```
trigger → flock-health-monitoring → [tool calls] → result → notify
```
...
```
trigger → feed-management-automation → [tool calls] → result → notify
```
...
```
trigger → biosecurity-compliance → [tool calls] → result → notify
```
...
```
trigger → production-tracking → [tool calls] → result → notify
```
