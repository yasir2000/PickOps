# 🔨 Construction Operations Agent — Workflows

## Workflow Patterns

This agent supports **4 primary workflow patterns**.

### Milestone Tracking Pipeline

```
trigger → milestone-tracking-pipeline → [tool calls] → result → notify
```
...
```
trigger → safety-alert-system → [tool calls] → result → notify
```
...
```
trigger → material-logistics-automation → [tool calls] → result → notify
```
...
```
trigger → budget-monitoring-report → [tool calls] → result → notify
```
