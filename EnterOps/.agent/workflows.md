# 🎭 Entertainment Operations Agent — Workflows
...
```
trigger → content-production-pipeline → [tool calls] → result → notify
```
...
```
trigger → rights-management-workflow → [tool calls] → result → notify
```
...
```
trigger → audience-analytics-report → [tool calls] → result → notify
```
...
```
trigger → live-event-operations → [tool calls] → result → notify
```
