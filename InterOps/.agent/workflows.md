# 🔗 Interoperability Operations Agent — Workflows
...
```
trigger → api-orchestration-pipeline → [tool calls] → result → notify
```
...
```
trigger → format-translation-flow → [tool calls] → result → notify
```
...
```
trigger → integration-test-suite → [tool calls] → result → notify
```
...
```
trigger → event-driven-integration → [tool calls] → result → notify
```
