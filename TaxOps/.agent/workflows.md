# 🧾 Tax Operations Agent — Workflows
...
```
trigger → tax-compliance-pipeline → [tool calls] → result → notify
```
...
```
trigger → multi-jurisdiction-calculation → [tool calls] → result → notify
```
...
```
trigger → filing-automation → [tool calls] → result → notify
```
...
```
trigger → audit-trail-management → [tool calls] → result → notify
```
