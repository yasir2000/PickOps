# 🌍 Social Operations Agent — Workflows
...
```
trigger → community-management-pipeline → [tool calls] → result → notify
```
...
```
trigger → grant-lifecycle-tracking → [tool calls] → result → notify
```
...
```
trigger → impact-measurement-report → [tool calls] → result → notify
```
...
```
trigger → volunteer-coordination → [tool calls] → result → notify
```
