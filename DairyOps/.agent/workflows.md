# 🐄 Dairy Operations Agent — Workflows
...
```
trigger → herd-health-monitoring → [tool calls] → result → notify
```
...
### Milking Schedule Automation

```
trigger → milking-schedule-automation → [tool calls] → result → notify
```
...
### Milk Quality Pipeline

```
trigger → milk-quality-pipeline → [tool calls] → result → notify
```
...
```
trigger → cold-chain-alert → [tool calls] → result → notify
```
