# 🏺 Archaeological Operations Agent — Workflows

## Workflow Patterns

This agent supports **4 primary workflow patterns**.

### Artifact Ingest Classify

```
trigger → artifact-ingest-classify → [tool calls] → result → notify
```
...
### Site Mapping Pipeline

```
trigger → site-mapping-pipeline → [tool calls] → result → notify
```
...
```
trigger → heritage-compliance-check → [tool calls] → result → notify
```
...
```
trigger → research-export → [tool calls] → result → notify
```
