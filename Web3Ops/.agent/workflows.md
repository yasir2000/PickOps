# 🌐 Web3 Operations Agent — Workflows
...
```
trigger → defi-protocol-operations → [tool calls] → result → notify
```
...
```
trigger → nft-platform-management → [tool calls] → result → notify
```
...
```
trigger → dao-governance-workflow → [tool calls] → result → notify
```
