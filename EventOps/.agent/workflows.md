# 📅 Event Operations Agent — Workflows
...
```
trigger → registration-automation → [tool calls] → result → notify
```
...
```
trigger → ticketing-pipeline → [tool calls] → result → notify
```
...
```
trigger → event-communication-flow → [tool calls] → result → notify
```
...
```
trigger → post-event-analytics → [tool calls] → result → notify
```
