# 📡 Communications Operations Agent — Workflows
...
```
trigger → message-queue-monitoring → [tool calls] → result → notify
```
...
```
trigger → email-deliverability-tracking → [tool calls] → result → notify
```
...
### Notification Routing Pipeline

```
trigger → notification-routing-pipeline → [tool calls] → result → notify
```
...
```
trigger → compliance-archive → [tool calls] → result → notify
```
