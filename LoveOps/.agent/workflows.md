# ❤️ Relationship & Wellbeing Operations Agent — Workflows
...
```
trigger → matching-pipeline → [tool calls] → result → notify
```
...
### Coaching Content Routing

```
trigger → coaching-content-routing → [tool calls] → result → notify
```
...
### Safety Moderation Loop

```
trigger → safety-moderation-loop → [tool calls] → result → notify
```
...
```
trigger → engagement-analytics → [tool calls] → result → notify
```
