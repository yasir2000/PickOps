# 🔋 Reliability Operations Agent — Workflows
...
```
trigger → slo-monitoring-pipeline → [tool calls] → result → notify
```
...
```
trigger → error-budget-tracking → [tool calls] → result → notify
```
...
```
trigger → incident-management-loop → [tool calls] → result → notify
```
