# 🔒 Development Security Operations Agent — Workflows
...
```
trigger → sast-pipeline → [tool calls] → result → notify
```
...
```
trigger → container-scan-gate → [tool calls] → result → notify
```
...
```
trigger → secrets-detection-alert → [tool calls] → result → notify
```
...
```
trigger → vulnerability-triage → [tool calls] → result → notify
```
