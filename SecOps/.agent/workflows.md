# 🔐 Security Operations Agent — Workflows
...
```
trigger → threat-detection-pipeline → [tool calls] → result → notify
```
...
```
trigger → incident-response-playbook → [tool calls] → result → notify
```
...
```
trigger → vulnerability-management-cycle → [tool calls] → result → notify
```
...
```
trigger → threat-intel-processing → [tool calls] → result → notify
```
