# 🎵 Music Operations Agent — Workflows
...
```
trigger → music-distribution-pipeline → [tool calls] → result → notify
```
...
### Royalty Tracking Workflow

```
trigger → royalty-tracking-workflow → [tool calls] → result → notify
```
...
```
trigger → artist-analytics-report → [tool calls] → result → notify
```
...
```
trigger → metadata-management → [tool calls] → result → notify
```
