# ⚖️ Compliance Operations Agent — Workflows
...
```
trigger → continuous-compliance-scan → [tool calls] → result → notify
```
...
```
trigger → audit-evidence-collection → [tool calls] → result → notify
```
...
```
trigger → risk-assessment-pipeline → [tool calls] → result → notify
```
...
```
trigger → regulatory-report-generation → [tool calls] → result → notify
```
