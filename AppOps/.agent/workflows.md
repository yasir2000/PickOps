# 📱 Application Operations Agent — Workflows
...
```
trigger → blue-green-deploy → [tool calls] → result → notify
```
...
```
trigger → canary-release → [tool calls] → result → notify
```
...
```
trigger → error-triage-loop → [tool calls] → result → notify
```
...
```
trigger → ab-experiment-lifecycle → [tool calls] → result → notify
```
