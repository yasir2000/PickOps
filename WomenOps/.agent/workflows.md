# 👩‍💻 Women in Tech & DEI Operations Agent — Workflows
...
```
trigger → pay-equity-analysis → [tool calls] → result → notify
```
...
```
trigger → inclusive-hiring-pipeline → [tool calls] → result → notify
```
...
```
trigger → mentorship-management → [tool calls] → result → notify
```
...
```
trigger → dei-metrics-reporting → [tool calls] → result → notify
```
