# 🚚 Logistics Operations Agent — Workflows
...
```
trigger → route-optimization-engine → [tool calls] → result → notify
```
...
### Warehouse Management Pipeline

```
trigger → warehouse-management-pipeline → [tool calls] → result → notify
```
...
```
trigger → shipment-tracking → [tool calls] → result → notify
```
...
```
trigger → last-mile-coordination → [tool calls] → result → notify
```
