# 🌐 National Operations Agent — Workflows
...
```
trigger → digital-identity-pipeline → [tool calls] → result → notify
```
...
```
trigger → cross-ministry-integration → [tool calls] → result → notify
```
...
```
trigger → cybersecurity-governance → [tool calls] → result → notify
```
