# ⚡ Energy Operations Agent — Workflows
...
```
trigger → grid-monitoring-pipeline → [tool calls] → result → notify
```
...
### Renewable Dispatch Loop

```
trigger → renewable-dispatch-loop → [tool calls] → result → notify
```
...
```
trigger → demand-forecast-model → [tool calls] → result → notify
```
...
```
trigger → carbon-accounting-report → [tool calls] → result → notify
```
