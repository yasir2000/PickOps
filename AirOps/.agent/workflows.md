# ✈️ Aviation Operations Agent — Workflows
...
```
trigger → crew-scheduling-optimization → [tool calls] → result → notify
```
...
```
trigger → maintenance-tracking → [tool calls] → result → notify
```
...
```
trigger → notam-parsing-alert → [tool calls] → result → notify
```
...
```
trigger → safety-audit-pipeline → [tool calls] → result → notify
```
