# 🤖 AI Agent Operations Agent — Workflows

## Workflow Patterns

This agent supports **5 primary workflow patterns**.

### Supervisor Subagent

```
trigger → supervisor-subagent → [tool calls] → result → notify
```
...
```
trigger → sequential-chain → [tool calls] → result → notify
```
...
```
trigger → parallel-fan-out → [tool calls] → result → notify
```
...
```
trigger → human-in-the-loop → [tool calls] → result → notify
```
...
```
trigger → reflection-loop → [tool calls] → result → notify
```
