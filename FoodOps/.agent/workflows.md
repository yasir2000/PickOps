# 🍽️ Food Operations Agent — Workflows

## Workflow Patterns

This agent supports **4 primary workflow patterns**.

### Haccp Monitoring Pipeline

```
trigger → haccp-monitoring-pipeline → [tool calls] → result → notify
```
...
```
trigger → production-scheduling → [tool calls] → result → notify
```
...
```
trigger → qc-automation → [tool calls] → result → notify
```
...
```
trigger → traceability-tracking → [tool calls] → result → notify
```
