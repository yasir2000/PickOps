# 🔷 Azure Cloud Operations Agent — Workflows
...
```
trigger → arm-deployment-pipeline → [tool calls] → result → notify
```
...
```
trigger → policy-compliance-scan → [tool calls] → result → notify
```
...
```
trigger → aks-operations-loop → [tool calls] → result → notify
```
