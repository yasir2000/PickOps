# ⚖️ Legal Operations Agent — Workflows
...
```
trigger → contract-lifecycle-pipeline → [tool calls] → result → notify
```
...
```
trigger → matter-management-workflow → [tool calls] → result → notify
```
...
```
trigger → billing-automation → [tool calls] → result → notify
```
...
```
trigger → document-review-pipeline → [tool calls] → result → notify
```
