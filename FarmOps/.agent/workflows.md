# 🚜 Farm Operations Agent — Workflows
...
```
trigger → crop-planning-optimization → [tool calls] → result → notify
```
...
### Equipment Tracking Pipeline

```
trigger → equipment-tracking-pipeline → [tool calls] → result → notify
```
...
```
trigger → supply-chain-management → [tool calls] → result → notify
```
...
```
trigger → financial-reporting → [tool calls] → result → notify
```
