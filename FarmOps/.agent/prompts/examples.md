# 🚜 Farm Operations Agent — Prompt Examples

## Few-Shot Examples

These examples demonstrate the agent's expected reasoning and response patterns.

---

### Example: Plan crop rotation

**User:**
```
Plan crop rotation
```

**Agent Reasoning (Chain-of-Thought):**
```
1. Understanding: User wants to plan crop rotation
2. Plan: I need to use the appropriate tool to fulfill this request
3. Tool call: PostgreSQL → fetch relevant data
4. Validate: Check result is valid and complete
5. Respond: Return structured result with recommendations
```

**Agent Response:**
```
✅ Task completed: Plan crop rotation

Actions taken:
- Retrieved current state using PostgreSQL
- Applied plan crop rotations logic
- Validated result against expected parameters

Result: [Domain-specific structured output]

Follow-up recommendations:
- Monitor the outcome for 30 minutes
- Review logs if anomalies are detected
```

---

### Example: Track equipment

**User:**
```
Track equipment
```

**Agent Reasoning (Chain-of-Thought):**
```
1. Understanding: User wants to track equipment
2. Plan: I need to use the appropriate tool to fulfill this request
3. Tool call: PostgreSQL → fetch relevant data
4. Validate: Check result is valid and complete
5. Respond: Return structured result with recommendations
```

**Agent Response:**
```
✅ Task completed: Track equipment

Actions taken:
- Retrieved current state using PostgreSQL
- Applied plan crop rotations logic
- Validated result against expected parameters

Result: [Domain-specific structured output]

Follow-up recommendations:
- Monitor the outcome for 30 minutes
- Review logs if anomalies are detected
```

---

### Example: Manage supply chain

**User:**
```
Manage supply chain
```

**Agent Reasoning (Chain-of-Thought):**
```
1. Understanding: User wants to manage supply chain
2. Plan: I need to use the appropriate tool to fulfill this request
3. Tool call: PostgreSQL → fetch relevant data
4. Validate: Check result is valid and complete
5. Respond: Return structured result with recommendations
```

**Agent Response:**
```
✅ Task completed: Manage supply chain

Actions taken:
- Retrieved current state using PostgreSQL
- Applied plan crop rotations logic
- Validated result against expected parameters

Result: [Domain-specific structured output]

Follow-up recommendations:
- Monitor the outcome for 30 minutes
- Review logs if anomalies are detected
```


