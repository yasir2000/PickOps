# 🏛️ Architecture & Design Operations Agent — Workflows
...
```
trigger → bim-version-review → [tool calls] → result → notify
```
...
```
trigger → compliance-check-pipeline → [tool calls] → result → notify
```
...
```
trigger → multi-discipline-coordination → [tool calls] → result → notify
```
...
```
trigger → drawing-package-export → [tool calls] → result → notify
```
