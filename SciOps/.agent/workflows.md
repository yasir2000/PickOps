# 🔭 Scientific Operations Agent — Workflows
...
```
trigger → scientific-workflow-automation → [tool calls] → result → notify
```
...
```
trigger → hpc-job-scheduling → [tool calls] → result → notify
```
...
```
trigger → experiment-tracking → [tool calls] → result → notify
```
...
```
trigger → simulation-pipeline → [tool calls] → result → notify
```
