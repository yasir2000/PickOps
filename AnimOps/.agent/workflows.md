# 🎬 Animation & Media Operations Agent — Workflows
...
```
trigger → render-job-submission → [tool calls] → result → notify
```
...
### Asset Version Pipeline

```
trigger → asset-version-pipeline → [tool calls] → result → notify
```
...
```
trigger → qa-frame-review → [tool calls] → result → notify
```
...
```
trigger → production-daily-report → [tool calls] → result → notify
```
