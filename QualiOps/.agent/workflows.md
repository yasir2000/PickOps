# ✅ Quality Operations Agent — Workflows

## Workflow Patterns

This agent supports **4 primary workflow patterns**.

### Inspection Automation Pipeline

```
trigger → inspection-automation-pipeline → [tool calls] → result → notify
```
...
```
trigger → non-conformance-management → [tool calls] → result → notify
```
...
```
trigger → corrective-action-workflow → [tool calls] → result → notify
```
...
```
trigger → audit-management → [tool calls] → result → notify
```
