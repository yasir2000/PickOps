# 💼 Business Operations Agent — Workflows
...
```
trigger → kpi-data-pipeline → [tool calls] → result → notify
```
...
```
trigger → okr-progress-tracker → [tool calls] → result → notify
```
...
```
trigger → approval-automation → [tool calls] → result → notify
```
...
```
trigger → board-report-generation → [tool calls] → result → notify
```
