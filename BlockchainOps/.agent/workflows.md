# ⛓️ Blockchain Operations Agent — Workflows
...
```
trigger → contract-deploy-verify → [tool calls] → result → notify
```
...
```
trigger → on-chain-event-listener → [tool calls] → result → notify
```
...
```
trigger → node-health-monitoring → [tool calls] → result → notify
```
...
```
trigger → wallet-transaction-pipeline → [tool calls] → result → notify
```
