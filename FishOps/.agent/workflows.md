# 🐟 Fishery Operations Agent — Workflows
...
```
trigger → fleet-tracking-pipeline → [tool calls] → result → notify
```
...
```
trigger → stock-assessment-model → [tool calls] → result → notify
```
...
```
trigger → aquaculture-monitoring → [tool calls] → result → notify
```
...
```
trigger → quota-compliance-report → [tool calls] → result → notify
```
