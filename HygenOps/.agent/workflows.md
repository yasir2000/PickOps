# 🧼 Hygiene Operations Agent — Workflows
...
```
trigger → sanitation-scheduling → [tool calls] → result → notify
```
...
```
trigger → audit-pipeline → [tool calls] → result → notify
```
...
```
trigger → compliance-monitoring → [tool calls] → result → notify
```
...
```
trigger → supply-reorder-trigger → [tool calls] → result → notify
```
