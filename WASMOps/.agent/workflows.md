# 🕸️ WebAssembly Operations Agent — Workflows
...
```
trigger → module-deployment-pipeline → [tool calls] → result → notify
```
...
```
trigger → edge-deployment-workflow → [tool calls] → result → notify
```
...
```
trigger → performance-profiling → [tool calls] → result → notify
```
...
```
trigger → security-sandbox-enforcement → [tool calls] → result → notify
```
