# 📖 Saga Pattern Operations Agent — Workflows
...
```
trigger → saga-orchestration-pipeline → [tool calls] → result → notify
```
...
```
trigger → compensation-management → [tool calls] → result → notify
```
...
```
trigger → event-sourcing-pipeline → [tool calls] → result → notify
```
...
```
trigger → distributed-state-tracking → [tool calls] → result → notify
```
