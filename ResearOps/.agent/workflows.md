# 🔬 Research Operations Agent — Workflows
...
```
trigger → data-collection-pipeline → [tool calls] → result → notify
```
...
```
trigger → analysis-automation → [tool calls] → result → notify
```
...
```
trigger → literature-review-workflow → [tool calls] → result → notify
```
...
```
trigger → publication-submission → [tool calls] → result → notify
```
