# 🚆 Transportation Operations Agent — Workflows
...
```
trigger → schedule-optimization-pipeline → [tool calls] → result → notify
```
...
```
trigger → real-time-tracking → [tool calls] → result → notify
```
...
### Passenger Experience Automation

```
trigger → passenger-experience-automation → [tool calls] → result → notify
```
...
```
trigger → revenue-management → [tool calls] → result → notify
```
