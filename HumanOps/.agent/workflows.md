# 👥 Human Resources Operations Agent — Workflows
...
```
trigger → recruitment-pipeline → [tool calls] → result → notify
```
...
```
trigger → onboarding-automation → [tool calls] → result → notify
```
...
```
trigger → performance-review-cycle → [tool calls] → result → notify
```
...
```
trigger → hr-compliance-audit → [tool calls] → result → notify
```
