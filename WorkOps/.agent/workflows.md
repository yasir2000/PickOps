# 🏢 Workplace Operations Agent — Workflows
...
```
trigger → space-booking-automation → [tool calls] → result → notify
```
...
```
trigger → facilities-management → [tool calls] → result → notify
```
...
```
trigger → visitor-management-flow → [tool calls] → result → notify
```
...
```
trigger → hybrid-work-coordination → [tool calls] → result → notify
```
