# 🏠 Home Operations Agent — Workflows
...
```
trigger → home-automation-routines → [tool calls] → result → notify
```
...
```
trigger → energy-optimization-loop → [tool calls] → result → notify
```
...
```
trigger → security-monitoring-alert → [tool calls] → result → notify
```
...
```
trigger → maintenance-scheduler → [tool calls] → result → notify
```
