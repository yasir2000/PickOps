# 🧒 Kids & Family Operations Agent — Workflows
...
```
trigger → content-safety-pipeline → [tool calls] → result → notify
```
...
```
trigger → parental-control-management → [tool calls] → result → notify
```
...
```
trigger → activity-scheduling → [tool calls] → result → notify
```
...
```
trigger → progress-tracking → [tool calls] → result → notify
```
