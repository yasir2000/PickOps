# 🔬 Microservices Operations Agent — Workflows
...
```
trigger → service-mesh-deployment → [tool calls] → result → notify
```
...
```
trigger → distributed-tracing-pipeline → [tool calls] → result → notify
```
...
```
trigger → chaos-experiment-runner → [tool calls] → result → notify
```
...
```
trigger → api-gateway-config → [tool calls] → result → notify
```
