# 🏛️ Government Operations Agent — Workflows

## Workflow Patterns

This agent supports **4 primary workflow patterns**.

### Citizen Service Pipeline

```
trigger → citizen-service-pipeline → [tool calls] → result → notify
```
...
```
trigger → policy-compliance-monitoring → [tool calls] → result → notify
```
...
```
trigger → inter-agency-coordination → [tool calls] → result → notify
```
...
```
trigger → open-data-publishing → [tool calls] → result → notify
```
